...
from rtw_models.commons.positional_encoding import RotaryEmbedding, SinusoidalPositionalEncoding
...
class TestPositionalEncodings:
    ...
    def test_rotary_embedding_is_executable(self):
        batch_size, n_heads, seq_len, d_model = 2, 2, 3, 4
        dropout = 0.0
        device = "cpu"

        encoding = RotaryEmbedding(d_model=d_model, dropout=dropout, max_len=seq_len, device=device)
        x = torch.ones((batch_size, n_heads, seq_len, d_model))
        output = encoding(x)
